...
PROMPT = """
以降のユーザープロンプトで示されるテキストは半導体露光装置における{basename}といういくつかの機能仕様書のサマリーです。

## 機能仕様書サマリー
{markdown_text}

このサマリーを統合して「{basename}」の現行仕様の解説ドキュメントを作成してください。

"""

INPUT_DIR = r'summary'
OUTPUT_DIR = r'synthesized_summary'
...
def synthesize_summary(rel_dir_path: str, input_dir: str, client: AzureOpenAI) -> str:
    """
...
def main():
    ...
    for undescribed in sorted(undescribeds):
        try:
            ...
            # 解説書を生成
            output = synthesize_summary(undescribed, INPUT_DIR, client)
